...
export default function EventCard({
  ...
  return (
    <>
      <Card className="flex min-h-[420px] flex-col rounded-[2rem] border-slate-200 bg-white p-8 shadow-md transition duration-300 hover:-translate-y-1 hover:shadow-xl">
        <div className="flex items-start gap-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="mt-4 text-2xl font-black leading-tight text-[#183972]">
              {title}
            </h3>
          </div>
        </div>
        ...
        <Button
          ...
          className="mt-6 w-fit rounded-full bg-[#183972] px-6 text-white hover:bg-[#102a58]"
        >
          Ver detalle
        </Button>
      </Card>
      ...
      {openDetail &&
        createPortal(
          <div className="fixed inset-0 z-[9999] flex h-screen w-screen items-center justify-center bg-black/40 px-4 backdrop-blur-xl">
            <Card className="relative w-full max-w-2xl rounded-[2rem] border-slate-200 bg-white p-8 shadow-2xl md:p-10">
              <Button
                onClick={() => setOpenDetail(false)}
                className="absolute right-5 top-5 rounded-full bg-[#183972] px-5 text-white hover:bg-[#102a58]"
              >
                Cerrar
              </Button>
